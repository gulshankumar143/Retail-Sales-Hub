...
const Dashboard = () => {
  ...
  const { data: summary = {}, isLoading: isSummaryLoading } = summaryQuery;
  const { data: charts = {}, isLoading: isChartsLoading } = chartsQuery;
  const { isLoading: isFiltersLoading } = filtersQuery;

  const recordCount = sales.length;
  const isBusy = isTableLoading || isSummaryLoading || isChartsLoading || isFiltersLoading;
  ...
  const headerSubtitle = useMemo(() => {
    if (isBusy) return 'Loading current sales insights...';
    return `${recordCount.toLocaleString()} record${recordCount === 1 ? '' : 's'} matching your filters`;
  }, [recordCount, isBusy]);
      ...
      <div className="space-y-8">
        <div className="rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-xl dark:border-slate-800/70 dark:bg-slate-950/80">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm font-medium uppercase tracking-[0.24em] text-indigo-500">Retail Sales Hub</p>
              <h1 className="mt-3 text-3xl font-semibold text-slate-900 dark:text-white">Sales analytics overview</h1>
        ...
        <AnalyticsCards summary={summary} isLoading={isSummaryLoading} />
        ...
        <div className="grid gap-8 xl:grid-cols-[320px_1fr]">
          ...
          <section className="space-y-6">
            <ChartsSection charts={charts} isLoading={isChartsLoading} />

            {isTableLoading && <LoadingState />}
            {isError && (
              <ErrorState
                message={error?.message || 'Failed to load sales data.'}
              />
            )}            
            {!isTableLoading && !isError && sales.length === 0 && <EmptyState />}
            {!isTableLoading && !isError && sales.length > 0 && (
              <>
