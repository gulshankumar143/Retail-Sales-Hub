...
const renderChart = (item, data) => {
  ...
  if (!chartData.length) {
    return (
      <div className="h-72 grid place-items-center text-sm text-slate-500 dark:text-slate-400">
...
const ChartsSection = ({ charts = {}, isLoading = false }) => {
  if (isLoading) {
    return (
      <div className="grid gap-6 md:grid-cols-2">
        {Array.from({ length: 4 }).map((_, index) => (
          <div
            key={index}
            className="h-72 animate-pulse rounded-3xl bg-slate-100 dark:bg-slate-800"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      {chartItems.map((item) => (
        <section
          key={item.title}
          className="rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-xl dark:border-slate-800/70 dark:bg-slate-950/80"
        >
